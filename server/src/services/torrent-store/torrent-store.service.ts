...
export class TorrentStoreService {
  ...
  private checkServer() {
    if (this.torrentServerInstance === null && process.env.NODE_ENV === 'production') {
      throw Error(
        'The torrent server is not running. You need to initialize it first.',
      );
    }
  }
  ...
  public deleteUnnecessaryTorrents = async () => {
    ...
    for (const infoHash of deletableInfoHashes) {
      try {
        ...
        if (!torrent) {
          console.log(
            `Skipping deletion of ${infoHash} because it is not present in the torrent store.`,
          );
          continue;
        }
        ...
        console.error(`Failed to delete torrent ${infoHash}: ${message}`);
      }
    }
  };
}
