...
> [!NOTE]
> This workspace is intended for local customization. Repository/CI links have been removed so you can adapt it freely.
...
### Docker quickstart (local image)

Build the image:

```sh
docker build -t stremio-ncore-addon:local .
```

Run it (adjust the host path for your data):
...
docker run --rm \
  ...
  -p 42069:42069/tcp \
  -p 42069:42069/udp \
  -v /path/on/host/stremio-ncore-addon:/addon \
  stremio-ncore-addon:local
```

Notes:

- `3000` is HTTP, `3443` is HTTPS (used by local-only mode).
- `42069` is used for torrent seeding; exposing both TCP and UDP is recommended.
- All persistent data is stored under `/addon` inside the container.
