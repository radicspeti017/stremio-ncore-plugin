...
services:
  stremio-ncore-addon:
    image: stremio-ncore-addon:local
    environment:
      - NCORE_USERNAME=
      - NCORE_PASSWORD=
    ports:
      ...
      - target: 3443 # Port for HTTPS (if you handle HTTPS yourself, you don't need this exposed)
        published: 3443
        protocol: tcp
      - target: 42069 # Port for torrent seeding (TCP + UDP)
        published: 42069
        protocol: tcp
    volumes:
      - type: bind
...
| `NCORE_COOKIE`  | Optional. A full `Cookie` header value from an already authenticated browser session. Only needed if programmatic user/pass login is blocked in your environment.                                                                                                                                    |
...
- torrent seeding uses port 42069, don't forget to expose this (TCP + UDP) to seed torrents.
